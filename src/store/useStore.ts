...
interface AppState {
    ...
    // Market
    fetchMarketCars: (type: 'new' | 'used', leagueId?: string) => Promise<void>;
    ...
    // Liga
    fetchLeagueParticipants: (leagueId: string) => Promise<any[]>;
}

export const useStore = create<AppState>((set, get) => {
    const lastFetchParams = { seed: -1, type: '' as 'new' | 'used' | '', leagueId: '' };
    ...
    createEventRequest: async (crewId: string, eventData: any) => {
        ...
        fetchMarketCars: async (type: 'new' | 'used', leagueId?: string) => {
            try {
                // CACHE CHECK: If same parameters, skip fetch
                const cacheKeyLeague = leagueId || '';
                const cacheKeyType = type;
                if (lastFetchParams.seed === 0 && lastFetchParams.type === cacheKeyType && lastFetchParams.leagueId === cacheKeyLeague) {
                    console.log(`[Store] Skipping Market Fetch (Cached) for ${type} / ${leagueId}`);
                    return;
                }

                console.log(`[Store] Fetching Market via RPC (Type: ${type}, League: ${leagueId})`);

                if (!leagueId) {
                    // console.warn('[Store] No leagueId provided for market fetch');
                    return;
                }

                // CALL RPC
                const { data, error } = await supabase.rpc('get_daily_market', {
                    p_league_id: leagueId,
                    p_type: type
                });

                if (error) {
                    console.error('[Store] fetchMarketCars RPC ERROR:', error);
                    throw error;
                }

                // Map raw data (Same robust parsing logic)
                const mappedCars: any[] = (data || []).map((row: any) => { // row is now any because we use RPC result
                    let imgs: string[] = [];
                    ...
                    if (row.power) {
                        const val = parsePowerString(row.power);
                        if (val) hpVal = val;
                    }
                    if (hpVal === 100 && (!row.power || !parsePowerString(row.power))) {
                        if (row.engine_specs_title) {
                            const val = parsePowerString(row.engine_specs_title);
                            if (val) hpVal = val;
                        }
                        if (hpVal === 100 && row.title) {
                            const val = parsePowerString(row.title);
                            if (val) hpVal = val;
                        }
                    }
                ...
                // Update Cache (We use 0 as 'valid fetch done' for seed since we don't use seed anymore)
                lastFetchParams.seed = 0;
                lastFetchParams.type = type;
                lastFetchParams.leagueId = leagueId || '';

                set({ marketCars: mappedCars });
        ...
        fetchLeagueParticipants: async (leagueId: string) => {
            try {
                // Fetch participants first
                const { data: participants, error: participantsError } = await supabase
                    .from('league_participants')
                    .select('id, user_id, budget, created_at')
                    .eq('league_code', leagueId)
